...
unsigned char *mc2rw_write_page(usb_dev_handle *udev, int page)
{
	int i=0, p=0, ret=0;
	...
	unsigned char start_write_cmd[] = {0xaa, 0x42, 0x09, 0x00, 0x81, 0x23, page&0xFF, (page>>8)&0xFF, (page>>16)&0xFF, (page>>24)&0xFF, xor, 0x00, 0x00};
	unsigned char ecc_write_cmd[148] = {0xaa, 0x42, 0x90, 0x00, 0x81, 0x43, 0x10, 0x00, 0x00};
	unsigned char end_write_cmd[] = {0xaa, 0x42, 0x04, 0x00, 0x81, 0x81, 0x00, 0x00};

	mcrw_addr_debug(page);


	//mc2rw_reset_auth(udev);
	//mc2rw_read_spec(udev);


	/* start write page */
	printf("Starting Read Page\n");
	ret = usb_bulk_write(udev, 0x02, start_write_cmd, sizeof(start_write_cmd), 1000);
	if (ret != sizeof(start_write_cmd)) printf("Error!\n");
	...
	ret = usb_bulk_write(udev, 0x02, ecc_write_cmd, sizeof(end_write_cmd), 1000);
	ret = usb_bulk_read(udev, 0x81, temp_data, sizeof(temp_data), 1000);
	mcrw_debug(temp_data, ret);
	
	/* end write page */
	printf("End Read Page\n");
	ret = usb_bulk_write(udev, 0x02, end_write_cmd, sizeof(end_write_cmd), 1000);
	if (ret != sizeof(end_write_cmd)) printf("Error! ret: %d, size: %d\n", ret, sizeof(end_write_cmd));
	ret = usb_bulk_read(udev, 0x81, temp_data, sizeof(temp_data), 1000);
	mcrw_debug(temp_data, ret);
